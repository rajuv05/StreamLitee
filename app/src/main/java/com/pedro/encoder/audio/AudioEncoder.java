...
public class AudioEncoder extends BaseEncoder implements GetMicrophoneData {
  ...
  public void formatChanged(@NonNull MediaCodec mediaCodec, @NonNull MediaFormat mediaFormat) {
    Log.i(TAG, "--- Negotiated Audio MediaCodec Parameters ---");
    if (mediaFormat.containsKey(MediaFormat.KEY_SAMPLE_RATE)) {
        Log.i(TAG, "Sample Rate: " + mediaFormat.getInteger(MediaFormat.KEY_SAMPLE_RATE));
    }
    ...
    if (mediaFormat.containsKey(MediaFormat.KEY_CHANNEL_COUNT)) {
        Log.i(TAG, "Channels: " + mediaFormat.getInteger(MediaFormat.KEY_CHANNEL_COUNT));
    }
    Log.i(TAG, "---------------------------------------------");
